 ...
 * HashNew() must return NULL, and HashFile() and HashPubKey() must leave
 * their all-zero digest rather than silently returning something that looks
 * like success.
 *
...
static void test_HashFile_leaves_zero_digest_on_init_failure(void)
{
    if (!init_failure_forced)
    {
        return;
    ...
    HashFile(file, digest, HASH_METHOD_SHA256, false);
...
int main()
{
    ...
    const UnitTest tests[] =
    {
        ...
        unit_test(test_HashFile_leaves_zero_digest_on_init_failure),
